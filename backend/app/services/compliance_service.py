...
def evaluate_bidder_compliance(
    ...
    tender = (
        ...
        .filter(models.Tender.id == bidder.tender_id)
        .first()
    )

    if not tender:
        raise ValueError(
            f"Tender ID {bidder.tender_id} not found."
        )

    # ---------------------------------------------------------
    # 3. Fetch requirements and documents
    # ---------------------------------------------------------
    requirements = (
        db.query(models.Requirement)
        .filter(models.Requirement.tender_id == tender.id)
        .all()
    )

    documents = (
        db.query(models.Document)
        .filter(models.Document.bidder_id == bidder_id)
        .all()
    )

    # ---------------------------------------------------------
    # 4. Initialize verification connector
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 5. Remove previous verification results
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 6. Verify every tender requirement
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 8. Save or update compliance score
    # ---------------------------------------------------------
    existing_score = (
